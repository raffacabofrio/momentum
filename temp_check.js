        ...
        // ... existing DEV_COLORS and other vars ...
        const DEV_COLORS = {
            'VAL': '#166534',
            'WAN': '#facc15',
            'IDY': '#3b82f6',
        ...
        function loadData() {
            ...
            sprint.tickets.filter(t => t.status === 'done').forEach(t => {
                devScores[t.dev] = (devScores[t.dev] || 0) + (Number(t.pts) || 0);
            });
            ...
            const calc = (list) => {
                ...
                // keep delivered for retrocompatibility
            ...
            updateKPI('planejados', 0, trends.pDone); // Val is updated below
            updateKPI('removidos', curr.removed, trends.removed, true);
            updateKPI('furafilas', 0, trends.fDone, true); // Val is updated below
            ...
            devPerformance.sort((a, b) => {
                ...
                if (currentSort.col === 'pD') {
                    // Sort by delivered, then by total planned
                    if (vA !== vB) return currentSort.dir === 'asc' ? vA - vB : vB - vA;
                    vA = a.pT; vB = b.pT;
                }
                if (currentSort.col === 'fD') {
                    // Sort by delivered fura-fila, then by total fura-fila
                    if (vA !== vB) return currentSort.dir === 'asc' ? vA - vB : vB - vA;
                    vA = a.fT; vB = b.fT;
                }
            ...
            document.getElementById('dev-table-body').innerHTML = devPerformance.map(d => {
            ...
            const epics = Object.values(epicMap).sort((a, b) => {
                ...
                if (totalA !== totalB) return totalB - totalA; // Regra 1: Total planejado + fura-filas

                if (a.pTotal !== b.pTotal) return b.pTotal - a.pTotal; // Regra 2: Quem tem mais planejados

                const doneA = a.pDone + a.fDone;
                const doneB = b.pDone + b.fDone;
                return doneB - doneA; // Regra 3: Quem entregou mais
            });
            const openEpics = Array.from(document.querySelectorAll('.epic-item[open] summary span:first-child')).map(el => el.innerText);
            const scrollPos = window.scrollY;

            document.getElementById('epic-list-container').innerHTML = epics.map((e, i) => {
        ...
        function handleDrop(e, newStatus) {
            ...
            if (ticket) {
                // Movimentação Visual no DOM (Preserva Scroll)
                ...
                // Atualizar o resto da tela sem recarregar os épicos
                updateStatsOnly();
        ...
        function updateStatsOnly(ticketId, newStatus) {
            const selectedSprintId = document.getElementById('filter-sprint').value;
            const selectedDev = document.getElementById('filter-dev').value;
            const sprint = MOMENTUM_SPRINTS_DATA.find(s => s.id === selectedSprintId) || MOMENTUM_SPRINTS_DATA[MOMENTUM_SPRINTS_DATA.length - 1];
            ...
            // NÃO chamar renderCharts() aqui para evitar o pulo de scroll.
            // Os gráficos grandes serão atualizados no próximo loadData ou F5.
        }
        ...
        function editComment(ticketId) {
            ...
            document.getElementById('modal-ticket-info').innerText = `${ticket.id} - ${ticket.title}`;
            document.getElementById('modal-textarea').value = ticket.comment || '';
            document.getElementById('commentModal').style.display = 'flex';
            document.getElementById('modal-textarea').focus();

            // Configurar botão de salvar (uma única vez)
        ...
        function updateLogo() { mainLogo.src = body.classList.contains('dark-mode') ? 'logo-dark.png' : 'logo.jpg'; }
        if (localStorage.getItem('theme') === 'dark') { body.classList.add('dark-mode'); updateLogo(); }
        themeToggle.addEventListener('click', () => {
            body.classList.toggle('dark-mode');
            localStorage.setItem('theme', body.classList.contains('dark-mode') ? 'dark' : 'light');
            updateLogo(); renderCharts();
        });
        ...
        document.getElementById('btnSync').addEventListener('click', async () => {
            ...
            finally { btn.innerText = 'Sync Jira'; btn.disabled = false; }
        });

        initFilters(); loadData(); renderCharts();
